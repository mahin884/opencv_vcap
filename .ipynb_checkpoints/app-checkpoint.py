...
def capture_stream(args):
    ...
    # flag for single image
    image_flag = False
    # Check type of input from the args
    if args.i == 'CAM':
        args.i = 0
    ...
    # Video writer for the output video
    if not image_flag:
    ...
    # process frames until video ends or exits
    while cap.isOpened():
        # Read the next frame
        flag, frame = cap.read()
        if not flag:
    ...
    if not image_flag:
        out.release() # release writer out,cap
    cap.release()
